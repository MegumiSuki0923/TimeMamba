...
class Dataset_ETT_hour(Dataset):
    def __init__(self, root_path, flag='train', size=None,
                 ...
                 seasonal_patterns=None, train_stride=1):
        if size == None:
            self.seq_len = 24 * 4 * 4
            self.label_len = 24 * 4
            self.pred_len = 24 * 4
        ...
        # P3a：仅训练集窗口降采样；val/test 保持逐点窗口
        self.train_stride = train_stride if flag == 'train' else 1
        ...
        self.n_windows = (self.tot_len - 1) // self.train_stride + 1
    ...
    def __read_data__(self):
        ...
        self.data_x = torch.FloatTensor(data[border1:border2])
        self.data_y = torch.FloatTensor(data[border1:border2])
...
class Dataset_ETT_minute(Dataset):
    ...
    def __getitem__(self, index):
        ...
        r_begin = s_end - self.label_len
        r_end = r_begin + self.label_len + self.pred_len
        seq_x = self.data_x[s_begin:s_end, feat_id:feat_id + 1].copy()
        seq_y = self.data_y[r_begin:r_end, feat_id:feat_id + 1].copy()
